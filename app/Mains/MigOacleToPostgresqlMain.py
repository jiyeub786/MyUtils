from app.PyClass import PostgresqlClass as Postgresql, OracleClass as oracle
from app.PyClass.LoggerClass import logger

import time

start = time.time()  # 프로그램 시작 시간

targetTableListQuery = "select 'select * from eduoracle.'||table_name, 'test.'||lower(table_name)  ,COUNT_TABLE_TEXT_ORACLE TABLE_COUNT ,NUM_ROWS , TABLE_NAME ,CREAT_BIND_INSERT_TEXT_POSTGRE from eduoracle.mig_tbl_list   where TARGET_YN='Y' order by NUM_ROWS"
...
for rowTp in targetInfoList:
    resultDict = {"selectORACLE": list(rowTp)[0]
        ...
        , "insertBindStringPostgresql": list(rowTp)[5]
              }
...
for targetTableIndex, targetTableText in enumerate(migrationTargetInfoList):
    # init oracle02 for target table fetch each rows
    oracle02 = oracle.Oracle()
    ...
    totlDurationSecond = str(time.time() - start)[0:5] + 's' # 개별작업 소요시간
    logger.info( "---------------------------------------------------------------------------------------------------------------------")
    logger.info( " [ start time" + " => " + totlDurationSecond +
    ...
    insertPerRow = 10000#bulk insert할 버퍼의 크기
    insertBuffer = []
    counter = 1
    sleepTerm = 0
    failCount = 0
    ...
    postgresql01 = Postgresql.Postgresql()
    while fetchList: # 2) 오라클에서 추출한 테이블별로 1행씩 FECTH하여 MYSQL에 INSERT함
        ...
        if int(insertPerRow) == int(len(insertBuffer)) or int(counter) == int(rowCount):
             ...
             try :
                 # 2-3) 만들어진 스트링을 mysql에 입력한다
                 failCount = 0
                 postgresql01.insertmany(targetTableText["insertBindStringPostgresql"], insertBuffer)
                 postgresql01.commit()
